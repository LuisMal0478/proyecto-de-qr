...
const Login = () => {
  ...
  // Redirigir al dashboard o de donde venía
  ...
  const handleGoogleResponse = async (response) => {
    ...
    if (res.success) {
      setToast({ message: 'Sesión iniciada con Google', type: 'success' });
      setTimeout(() => {
        navigate(from, { replace: true });
      }, 800);
    } else {
      setToast({ message: res.message || 'Error al iniciar sesión con Google', type: 'error' });
    }
  };

  // Simular Google Sign In (Desarrollo offline sin Google Client ID)
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      ...
      setToast({ message: res.message || 'Error al iniciar sesión', type: 'error' });
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-dark-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <button
        onClick={toggleTheme}
        className="absolute top-5 right-5 z-20 p-2.5 bg-white dark:bg-dark-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-dark-850 transition-colors"
      >
      ...
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-br from-brand-600 to-indigo-700 dark:from-brand-950/60 dark:to-indigo-950/50 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="z-10 max-w-md">
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-4 leading-snug">
            Bienvenido de nuevo a la revolución de los códigos QR.
          </h2>
          <p className="text-brand-100 text-sm leading-relaxed">
            Inicia sesión para administrar tus códigos QR, editar tus enlaces dinámicos y supervisar las analíticas de tus visitas.
          </p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 relative">
        <div className="w-full max-w-md bg-white dark:bg-dark-900 p-8 sm:p-10 rounded-3xl border border-slate-200/60 dark:border-slate-800/40 shadow-xl shadow-slate-100/10 dark:shadow-black/25">
          <div className="mb-8">
            <h1 className="text-2xl font-black mb-1.5">Iniciar Sesión</h1>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-dark-900 px-3 text-slate-400 font-bold">O ingresar con</span>
            </div>
          </div>
